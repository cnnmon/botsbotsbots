  ...
  return (
    <div className="flex items-start mb-2">
      <Image
        src={profile}
        alt={name}
        className="w-14 h-14 button no-drag"
        onClick={() => openProfile(sender)}
      />
      <div className="pl-2 w-full">
        <p>{name}</p>
        <p
          className={`border-[1.5px] border-primary-color p-2 w-full whitespace-pre-wrap ${
...
export default function Chatroom({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      console.log('Enter pressed', chatboxText);
      e.preventDefault();
  ...
  return (
    <div className="h-full flex-col">
      <div
        className="overflow-y-auto p-2 no-drag"
        style={{
